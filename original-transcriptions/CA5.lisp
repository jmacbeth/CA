;;; CA5.lisp contains the functions for building and manipulating CDs.
...
(defmacro BUILD-CON (concept fillers equivalences)
  `(BUILD-CD ',concept ',fillers ',equivalences))


(defun BUILD-CD (concept &optional fillers equivalences) 
...
(defun SET-ROLE-FILLER (path concept filler)
  (let ((nc (make-cd filler))
        (dc (get-role-filler path concept)))
    (cond (oc (subst-cd nc oc concept)
              (chain-gap oc nc))  ;; in preds.lisp (propagage slot change)
...
(defun MAKE-CD (X) ;; was a DEX
  (let ((con (build-c x nil)))
    (cond ((neq x con)
           ...
           (and (is-running ca)
                (list-add CON  ;; (or (has-value 'DUMMY) CON)
                          'CHANGED-CONS))))
    CON))
